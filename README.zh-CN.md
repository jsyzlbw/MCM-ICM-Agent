...
当前实现是 CLI-first，同时有本地 FastAPI 后端作为未来 GUI 的基础。今天已经比较完整的是 workflow 和 artifact contract，不是生产级前端。
...
提供 config、provider-test、workspace、upload、status、artifact-list、artifact-read 和 artifact-download endpoints。当前仓库还没有生产级前端。
...
| `POST /api/config` | 保存本地 runtime config。 |
...
| `GET /api/workspaces/{workspace_id}/artifacts/download` | 下载 artifact。 |
