...
当前仓库已经有一套可运行的底层 workflow：

- `src/mcm_agent/cli.py`：Typer CLI。
- `src/mcm_agent/workflows/mvp.py`：主 workflow 组装。
- `src/mcm_agent/core/stage_executor.py`：stage 执行器。
- `src/mcm_agent/core/workflow_graph.py`：workflow 拓扑。
- `src/mcm_agent/agents/`：各阶段 Agent。
- `src/mcm_agent/providers/`：LLM、搜索、MinerU、官方数据、LaTeX、embedding 等 provider。
- `src/mcm_agent/solver_modules/`：确定性 solver baseline。
- `tests/`：覆盖 CLI、workflow、provider、RAG、solver、paper、server 等行为。

当前实现偏“开发者 CLI + workflow 引擎”。新的产品设计要求把它包装成“裸 `mag` 进入交互式
Agent”的用户体验。
