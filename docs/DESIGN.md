...
Problem Understanding Agent 理解题意、拆解子问题
        ↓
Data Feasibility Scout Agent 提前检查关键数据是否公开可得
        ↓
Research Reframing Agent 在关键数据私有或不可得时提出代理变量或替代研究路径
        ↓
User Discussion Agent 基于题意、数据可行性和重构选项与用户讨论论文思路
        ↓
...
当前实现采用一个更严格的早期数据门禁：系统不能让用户先确认一个看似漂亮但数据不可获得的研究方案。拿到题目后，Agent 会先对潜在关键数据做可行性侦察；如果发现数据可能私有、缺失、不可下载或无法合法引用，就先触发 Research Reframing，再进入用户讨论和方向锁定。
...
```text
  ...
  ┌───────────┼────────────┬──────────────┬─────────────────┼───────────┐
  │           │            │              │                 │           │
Intake   Data Scout    Modeling        Search/Data     Coder       Writer
MinerU   Reframing     Council         RAG/EDA         Validation  Reviewer
```
...
1. `problem.understanding.ready`
   - 用户确认题意和子问题拆解。
2. `data.feasibility.ready`
   - 系统展示关键数据是否可获得，以及必须采用代理变量或重构方案的地方。
3. `model.decision.ready`
   - 用户确认最终模型路线和论文框架。
4. `paper.draft.ready`
   - 用户审查初稿并提出修改意见。
5. `submission.ready`
...
数据可行性检查是用户讨论前的前置门禁，而不是后置补救。流程如下：

```text
Problem Understanding
        ↓
Data Feasibility Scout 生成 data_feasibility_matrix.json
        ↓
如果关键数据 available 或可用 proxy
        ↓
User Discussion Agent 进入方向讨论
        ↓
如果用户提出新数据需求
        ↓
discussion/data_questions.json
        ↓
Data Feasibility Scout 复查
        ↓
Research Reframing Agent 在 private_or_unavailable 时生成 reframing_options
        ↓
User Discussion Agent 锁定 direction_lock.json
```

该循环用于处理美赛中常见的数据现实问题。例如，球员真实工资奖金、公司内部运营数据、非公开医疗记录等往往无法可靠获取。系统不能基于这些数据制定最终论文路线，而应优先考虑公开代理变量、题目附件数据、用户明确假设或改写研究问题。
...
#### 4.4.1 Data Feasibility Scout Agent

负责在用户正式锁定论文路线前，检查关键数据是否现实可得。

职责：

- 从题意理解、初步建模方向和用户补充 idea 中抽取数据需求
- 对每个数据需求执行轻量搜索或 provider 检查
- 判断数据状态：`available`、`proxy_required`、`private_or_unavailable`、`unknown`
- 为不可获得数据提出代理变量、替代指标或重构建议
- 在用户讨论前输出数据可行性矩阵，避免后期发现方案无法实现
...
- 任何关键数据被判为 `private_or_unavailable` 时，不能直接进入方向锁定。
- 如果数据状态为 `unknown`，优先进入 Search & Data Agent 做深度检索。
- 如果用户提出新的数据依赖，必须回到 Data Feasibility Scout 复查。

#### 4.4.2 Research Reframing Agent

负责在关键数据不可获得时，重构研究问题或提出可公开支撑的代理变量路线。

职责：

- 分析不可获得数据对原始研究方案的影响
- 生成 2-4 个替代研究路线
- 为每个路线说明代理变量、数据来源、建模影响和论文叙事影响
- 帮助 User Discussion Agent 向用户解释为什么需要改题、改指标或改假设
...
- Reframing 不是降低目标，而是把方案改成可证据支撑、可复现、可写成论文的路线。
- 被采纳的重构选项必须写入 `discussion/direction_lock.json`，供后续 Modeling Council 和 Writer 使用。
...
- 摘要必须包含每个子问题的方法和关键结果
- 关键章节必须写入机器可读 trace 注释，例如 `evidence_id`、`figure_id`、`source_id`
- 重要论断必须逐步升级为 claim 级绑定，例如：
...
系统维护一组互相绑定的证据文件，而不是只维护单个证据列表。目标是让人工审查者可以从论文中的每个关键论断追溯到来源、数据、代码、图表和引用。

证据链分五层：

1. `data/source_registry.json`
   - 记录每个外部来源的网站、访问时间、provider、可信等级和引用建议。
2. `data/data_lineage.json`
   - 记录进入模型或图表的数据项，绑定 `datum_id` 和 `source_id`。
3. `results/evidence_registry.json`
   - 记录代码输出、指标、模型结果和可写入论文的结论证据。
4. `figures/figure_registry.json`
   - 记录每张图的生成脚本、输出格式、使用章节、来源数据和支持的 evidence。
5. `review/paper_evidence_bindings.json`
   - 记录论文章节和 claim 级 trace 是否有效。

`results/evidence_registry.json` 示例：
...
论文中的重要论断应逐步采用 claim 级机器注释：

```tex
% claim_id=claim_model_route evidence_id=metric_priority_score_mean figure_id=fig_priority_ranking source_id=web_001
```

`PaperEvidenceBindingAgent` 会检查：

- `evidence_id` 是否存在于 `results/evidence_registry.json`
- `figure_id` 是否存在于 `figures/figure_registry.json`
- `source_id` 是否存在于 `data/source_registry.json`
...
│   ├── data_questions.json
│   ├── direction_lock.json
│   ├── reframing_options.md
│   ├── reframing_options.json
...
│   ├── data_feasibility_decision.json
│   ├── data_profile.md
│   ├── model_candidates.md
│   ├── model_decision.md
│   ├── experiment_plan.md
...
当前仓库已经实现可运行 MVP：CLI、workspace、provider adapters、graph workflow、data feasibility、reframing、search governance、solver evidence、vector-first figures、paper writing、reference audit、claim-level evidence binding、humanization safety、review and submission packaging 都有测试覆盖。下一阶段不再是继续堆叠 Agent 名称，而是提高论文质量、建模深度和真实 API 覆盖。
...
- 用户交互主要是 CLI/file workflow，尚未实现完整 UI。
